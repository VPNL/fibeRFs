...
files <- dir(paste0(path), 
             pattern = "*_radialFits_linear_contraOnly_10_40.mat")
...
for (f in files) {
  ...
  if (grepl("lh",mf)) {
    lh_rsquared = data$Rsquared
    lh_int = data$intercept
    lh_slope = data$slope
  } else if (grepl("rh",mf)) {
    rh_rsquared = data$Rsquared
...
IOG <- data.frame(subject,hemi,stream,ROI,rh_slope[1,])
colnames(IOG) <- c("subject","hemi","stream","ROI","slope")
ROI <- rep(c("pFus"), times = dim(rh_slope)[2])
pFus <- data.frame(subject,hemi,stream,ROI,rh_slope[2,])
colnames(pFus) <- c("subject","hemi","stream","ROI","slope")
ROI <- rep(c("mFus"), times = dim(rh_slope)[2])
mFus <- data.frame(subject,hemi,stream,ROI,rh_slope[3,])
colnames(mFus) <- c("subject","hemi","stream","ROI","slope")

stream <- rep(c("blateral"), times = dim(rh_slope)[2])
ROI <- rep(c("pSTS"), times = dim(rh_slope)[2])
pSTS <- data.frame(subject,hemi,stream,ROI,rh_slope[4,])
colnames(pSTS) <- c("subject","hemi","stream","ROI","slope")
ROI <- rep(c("mSTS"), times = dim(rh_slope)[2])
mSTS <- data.frame(subject,hemi,stream,ROI,rh_slope[5,])
colnames(mSTS) <- c("subject","hemi","stream","ROI","slope")

stream <- rep(c("blateral"), times = dim(rh_slope)[2])
ROI <- rep(c("CoS"), times = dim(rh_slope)[2])
CoS <- data.frame(subject,hemi,stream,ROI,rh_slope[6,])
colnames(CoS) <- c("subject","hemi","stream","ROI","slope")
...
IOG <- data.frame(subject,hemi,stream,ROI,lh_slope[1,])
colnames(IOG) <- c("subject","hemi","stream","ROI","slope")
ROI <- rep(c("pFus"), times = dim(lh_slope)[2])
pFus <- data.frame(subject,hemi,stream,ROI,lh_slope[2,])
colnames(pFus) <- c("subject","hemi","stream","ROI","slope")
ROI <- rep(c("mFus"), times = dim(lh_slope)[2])
mFus <- data.frame(subject,hemi,stream,ROI,lh_slope[3,])
colnames(mFus) <- c("subject","hemi","stream","ROI","slope")

stream <- rep(c("blateral"), times = dim(lh_slope)[2])
ROI <- rep(c("pSTS"), times = dim(lh_slope)[2])
pSTS <- data.frame(subject,hemi,stream,ROI,lh_slope[4,])
colnames(pSTS) <- c("subject","hemi","stream","ROI","slope")
ROI <- rep(c("mSTS"), times = dim(lh_slope)[2])
mSTS <- data.frame(subject,hemi,stream,ROI,lh_slope[5,])
colnames(mSTS) <- c("subject","hemi","stream","ROI","slope")

stream <- rep(c("blateral"), times = dim(lh_slope)[2])
ROI <- rep(c("CoS"), times = dim(lh_slope)[2])
CoS <- data.frame(subject,hemi,stream,ROI,lh_slope[6,])
colnames(CoS) <- c("subject","hemi","stream","ROI","slope")
