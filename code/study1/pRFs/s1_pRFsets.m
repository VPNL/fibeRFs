...
norm = 0; 
voxThresh = 25; %minimum number of voxels per ROI
...
for h = 1:length(hems)
    ...
    for i = 1:length(sessions)
       ...
       view = rmSelect(view,1,rmPath);
        ...
        for m = 1:length(maps) % if sub has map, put pRF_cov into data, otherwise fill with NaNs
            ...
            subj(i).roi(m).fits.ppd = 30; %% FIX THIS!!!
            ...
            if exist(fullfile('/share', 'kalanit', 'biac2', 'kgs', 'projects', 'fibeRFs', 'data', 'study1', 'toon', sessions{i}, '3DAnatomy', 'ROIs', [maps{m}, '.mat'])) || exist(fullfile('/share', 'kalanit', 'biac2', 'kgs', 'projects', 'fibeRFs', 'data', 'study1', 'toon', sessions{i}, '3Danatomy', 'ROIs', [maps{m}, '.mat']))
                ...
                [pRF_COV, coIndices, figHandle, all_models, weight, data] = rmPlotCoverage_flips(view,'prf_size',1,'fieldRange',fieldRange,'method','maximum profile','nboot',50,'normalizeRange',0,'smoothSigma',1,'cothresh',ve_cutoff,'weight','variance explained','sigmathresh',sigThresh,'eccthresh',eThresh,'addcenters',1,'newfig',0, 'flips', flips(i));
                ...
                if ~isempty(data) %only if there are above threshold voxels
                    if length(data.subSize1) < voxThresh %and more of them than our set threshold
                        ...
                        subj(i).roi(m).fits(1).vox(1).mrvsize = NaN;
                        subj(i).roi(m).fits(1).vox(1).kksize = NaN;
                        ...
                        for v = 1:length(data.subSize1)
                            if data.subSize1(v) > sigThresh
                                subj(i).roi(m).fits(1).vox(v).size = data.subSize1(v); %prf size for that voxel
                                subj(i).roi(m).fits(1).vox(v).mrvsize = data.subSize1(v)./sqrt(data.exponent(v)); %to mirror s_rmPlotEccSigma
                                subj(i).roi(m).fits(1).vox(v).kksize = (2*data.subSize1(v))./sqrt(data.exponent(v)); %to mirror kendrick
                                ...
                                subj(i).roi(m).fits(1).vox(v).mrvsize = NaN;
                                subj(i).roi(m).fits(1).vox(v).kksize = NaN;
    ...
    saveFile = fullfile(dataDir,[hems{h} '_pRFset_' num2str(fieldRange) '_ve' num2str(ve_cutoff*100)]); 
