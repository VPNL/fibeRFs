...
for h = 1:length(hems) 
    ...
    missing = 0; %how many face ROIs is this subj missing
    
    for r = 1:length(ROIs) 
        
        % input and output ROI names for this hemisphere
        inROI = ['fibeRFs_f_' hems{h},'_' ROIs{r}];
    ...
    for r = 2:(length(ROIs)-missing)
        if ~exist(fullfile('3DAnatomy','ROIs',['fibeRFs_f_' hems{h},'_' ROIs{r}]))
            fprintf('Missing %s...\n',inROI);
        else
            ...
            vw = combineROIs(vw, {roiCurr roiExcl}, 'AnotB', strcat(newPre,hems{h},'_',ROIs{r}), [vw.ROIs(end).comments 'Overlap removed via unifyROI.m']);
