function [niftiRoiName, niftiRoi] = fs_labelFileToNiftiRoiGWMI(fs_subject,labelFileName,niftiRoiName,hemisphere,regMgzFile,smoothingKernel,fsSubDir,direction)
%
...
% Updated by DF 2019
% Added direction as an input
% If direction = 1, we extend the ROI into the gray matter
% If direction = -1, we go the other way and extend into the white matter
...
if notDefined('smoothingKernel'), smoothingKernel = 10;   end
...
if direction == -1 %into the white
    cmd = sprintf('!mri_label2vol --subject %s --label %s --o %s.nii.gz --hemi %s --reg %s.dat --temp %s --proj frac -1 0 .1 --fillthresh .1', ...
          fs_subject,labelFileName,niftiRoiName,hemisphere, tmpRegFile,  regMgzFile);
elseif direction == 1 %into the cortical gray
    cmd = sprintf('!mri_label2vol --subject %s --label %s --o %s.nii.gz --hemi %s --reg %s.dat --temp %s --proj frac 0 1 .1 --fillthresh .1', ...
          fs_subject,labelFileName,niftiRoiName,hemisphere, tmpRegFile,  regMgzFile);
end
eval(cmd);
% 
% cmd = sprintf('!mri_label2vol --subject %s --label %s --o %s.nii.gz --hemi %s --reg %s.dat --temp %s --proj frac -2 1 .1 --fillthresh .1', ...
%       fs_subject,labelFileName,niftiRoiName,hemisphere, tmpRegFile,  regMgzFile);
% eval(cmd);
...
%     niftiRoi.data  = single(dtiCleanImageMask(niftiRoi.data,smoothingKernel,1));
%     niftiRoiName   = sprintf(niftiRoiName);
%     niftiRoi.fname = strcat(niftiRoiName, '.nii.gz')
