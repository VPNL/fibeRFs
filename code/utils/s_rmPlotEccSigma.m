function data = s_rmPlotEccSigma(v, lr, fwhm, plot_pv, plotFlag, vethresh, eccthresh, sigthresh, voxthresh)
...
thresh.varexp = vethresh; %max(viewGet(v,'cothresh'), rmParams.analysis.fmins.vethresh);
...
ii = roi.varexp > thresh.varexp; %using only varexp to be consistent with other code -DF 2019
% ii = roi.varexp > thresh.varexp & ...
%     roi.ecc > thresh.ecc(1) & roi.ecc < thresh.ecc(2) & ...
%     roi.sigma > thresh.sig(1) & roi.sigma < thresh.sig(2);

if sum(ii) < voxthresh %threshold for minimum num voxels
    data.error = true;
    data.xfit = NaN;
    data.yfit = NaN;
    data.x = NaN;
    ...
    if exist('bootstrp', 'file') && sum(ii) > 1
    ...
    if plotFlag == 1,
        ...
        if exist('bootstrp', 'file') && length(ii) > 1
...
end % this end is for the size check to make sure we have enough useable voxels
return;
