function [RFcov, coIndices, figHandle, all_models, weight, data] = rmPlotCoverage_flips(vw, varargin)
...
%  flips:           0 = no flips, 1 = y-flip, 2 = x-flip, 3 = x and y flips
%  css:             0 = not using Kendrick's CSS pRF model, 1 = used CSS model
...
% 06/19 DF edited to allow for weird flip issues and added coindices (so
% can use sum(coIndices) and numel(coIndices) to check proportion of voxels
% invldued)
% 02/20 DF edited to return exponent values as well
% 04/20 DF edited to fix size estimate for CSS model
...
if strcmpi(varargin{1}, 'dialog')
    ...
    for ii = 1:2:length(varargin)
        switch lower(varargin{ii})
            ...
            case 'flips', vfc.flips = varargin{ii+1};
            case 'css', vfc.css = varargin{ii+1};
        end
    end
end
...
if vfc.smoothSigma
    ...
    % HACK ADDED BY DAWN SO CODE DOESN'T BREAK
    if size(subx0, 2) < 3
        fprintf(1, '[%s]:Not enough values above threshold.\n', mfilename);
        RFcov = zeros(vfc.nSamples);
        figHandle = [];
        all_models = [];
        weight = [];
...
RFcov = nanmean(mask, 3); %% CHANGED FROM NANSUM - DF 2020
