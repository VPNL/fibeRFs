...
function nii2label(subject,dataPath,roiList,roiVal,labelPath,hemi)
	...
	for roi = length(roiList)
        ...
        outpath = ['/biac2/kgs/3Danat/FreesurferSegmentations/', subject ,'/'];
...
% 		unix(cmd);
        ...
        for p = 1:length(proj_values)
            ...
            cmd = ['mri_vol2surf  --src ' outFile{1} ' --out ' p_out{1} ' --srcreg ' outpath 'surf/register.dat --hemi ' hemi ' --projfrac ' num2str(proj_values(p))];
            unix(cmd)
        end
        ...
        cmd = ['mri_cor2label --i ' mgzSave{1} ' --id ' num2str(roiVal(roi)) ' --l ' saveLabel{1} ' --surf ' subject ' ' hemi ' inflated '];
 		unix(cmd);
